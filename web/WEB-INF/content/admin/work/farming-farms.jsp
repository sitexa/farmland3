	...
	<script type="text/javascript">
		$(function(){
			$("tbody tr").each(function(){	
				$(this).click(function(){	
					self.location="<%=request.getContextPath()%>/admin/work/farming?farmId="+this.id;	
				})
			})
...
<div class="box">
	<div class="left">
		<div class="menu">
			...
			<div class="items">
				<ul>
					<li><a href="<%=request.getContextPath()%>/admin/work/work!farms?landId=${landId}">农庄列表</a></li>
					<li>菜单条二</li>
					<li>菜单条三</li>
					<li>菜单条四</li>
				</ul>
	...
	<div class="bar-right"><a href="Javascript:history.back()">后退</a></div>
		...
		<table id="farms" cellpadding="0" cellspacing="1">
		    <thead>
		        ...
		        <th width="220">农庄名称</th>
		        <th width="150">农庄主</th>
		        <th width="150">起租时间</th>
		        <th width="110">待处理指令</th>
			</thead>
			<tbody>
		    <s:if test="#request.farmList.size>0">
		        <s:iterator value="#request.farmList" var="object" status="its">
		            <tr id="${object[0]}">
						<td>${its.index+1}</td>
		                <td>${object[1]}</td>
		                <td>${object[2]}</td>
		                <td>${object[3]}</td>
		                <td><s:date name="#request.object[4]" format="yyyy.MM.dd  hh:ss"/></td>
		                <td><s:if test="#request.object[5]!=null">${object[5]}条</s:if> </td>
		            </tr>
		        </s:iterator>
		    </s:if>
			</tbody>
		...
		<s:form id="pageFarm" theme="simple" action="%{#request.contextPath}/admin/work/farming!farms" method="get" cssStyle="display: none">
			<s:hidden name="landId" value="%{#request.landId}"></s:hidden>
